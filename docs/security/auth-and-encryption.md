...
Multi-account background synchronization (Gmail sync, Calendar sync, Contacts sync) frequently executes in parallel across decoupled BullMQ worker threads — and, since the API server and the standalone worker process (`src/worker.ts`) can each be horizontally scaled, across multiple separate OS processes/replicas, not just multiple threads within one process. If an account's token is near expiry, concurrent callers could simultaneously trigger token refresh calls against Google's OAuth endpoints.

**Correction (2026-09-21):** this section previously claimed the in-memory mutex alone "guarantee[d] zero race conditions." That was true only *within a single process* — it did not protect two replicas from racing on the same account, which matters here because Google can rotate refresh tokens on use: if two replicas both read the same refresh token and both call Google concurrently, whichever call lands second can be handed `invalid_grant` for a token the other replica had just consumed a moment earlier, incorrectly flagging the account `status = 'error'` on a pure timing race, not a real revocation. A Redis-backed distributed lock (`src/utils/distributed-lock.ts`) was added to close this gap. The defense is now two layers:
...
2. **Single-Flight Concurrency Mutex (within one process)**: an in-memory promise map keyed by `accountId` (`activeOperations`) intercepts concurrent requests for the same account inside one process. The first caller initiates the Google refresh request; subsequent callers synchronously join and await the identical in-flight promise.
3. **Distributed Lock (across processes/replicas)**: before actually calling Google, `GoogleTokenManager` acquires a Redis `SET NX PX` lock keyed per account (`lock:google-oauth-refresh:<accountId>`), then re-reads the account row. If another replica already refreshed it while this one waited for the lock, it reuses the now-fresh token instead of calling Google again. Fails open (best-effort, no lock) if Redis itself is unavailable, so a Redis outage degrades safety rather than blocking sync entirely. See `token-manager-distributed-lock.test.ts` for a test proving mutual exclusion across two independent `GoogleTokenManager` instances.
4. **Terminal Error Recovery**: Catches `invalid_grant` or revoked token responses, automatically marking `connectedAccounts.status = 'error'` and `syncStates.status = 'error'`, triggering an audit alert for user re-authentication without crashing worker pipelines.
