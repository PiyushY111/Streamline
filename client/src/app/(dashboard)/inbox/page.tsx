...
function InboxContent() {
  ...
  const filteredEmails = emails.filter((email) => {
    ...
    if (activeFolder === 'inbox' && activeCategory !== 'all') {
      const emailCategory = (email as any).category?.toLowerCase() || 'primary';
      if (activeCategory === 'promotions' && !emailCategory.includes('promo') && !email.subject.toLowerCase().includes('offer')) {
        if (emailCategory !== 'promotions') return false;
      } else if (activeCategory === 'social' && !emailCategory.includes('social') && !email.sender.includes('linkedin') && !email.sender.includes('twitter')) {
        if (emailCategory !== 'social') return false;
      } else if (activeCategory === 'updates' && !emailCategory.includes('update') && !email.sender.includes('godaddy') && !email.sender.includes('google')) {
        if (emailCategory !== 'updates') return false;
      } else if (activeCategory === 'primary' && (emailCategory === 'promotions' || emailCategory === 'social')) {
        return false;
      }
    }
          ...
          <div className="flex-1 flex bg-white dark:bg-[#141517] overflow-hidden rounded-tl-2xl border-l border-slate-200/80 dark:border-slate-800 shadow-xs min-h-0">
            ...
            {(!isReadingThread || viewMode === 'split') && (
              ...
              >
                ...
                {/* Gmail Category Tabs Bar (Primary, Promotions, Social, Updates) */}
                {activeFolder === 'inbox' && (
                  <div className="flex items-center border-b border-slate-200/80 dark:border-slate-800 bg-[#f6f8fc]/60 dark:bg-[#1a1b1e] shrink-0 overflow-x-auto">
                    {[
                      ...
                      { id: 'all', label: 'All Mail', icon: Mail, color: 'border-slate-600 text-slate-600' },
                    ].map((cat) => {
                      const Icon = cat.icon;
                    ...
                    paginatedEmails.map((email) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          {email.accountName && (
                            <span
                              className="px-2 py-0.5 rounded-md text-[10px] font-semibold shrink-0 transition-transform hover:scale-105"
                              style={{
                                backgroundColor: `${email.accountColor || '#0b57d0'}15`,
                                color: email.accountColor || '#0b57d0',
                                border: `1px solid ${email.accountColor || '#0b57d0'}30`,
                              }}
                              title={`Mailbox: ${email.accountName}`}
                            >
                              {email.accountName}
                            </span>
                          )}
                          ...
                          <div className="flex-1 min-w-0 flex items-center space-x-2 truncate text-xs">
                  ...
                  <div className="flex items-center space-x-3">
                    ...
                    <span
                      ...
                      style={{
                        ...
                        border: `1px solid ${selectedEmail?.accountColor || '#0b57d0'}30`,
                      }}
                    >
