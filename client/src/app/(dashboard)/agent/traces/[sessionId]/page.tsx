...
import { GraphSkeleton, ListSkeleton } from '@/components/ui/Skeleton';
import { TraceHeaderKPIs } from '@/components/agent/traces/TraceHeaderKPIs';

const TraceTimeline = dynamic(
  () => import('@/components/agent/traces/TraceTimeline').then((mod) => mod.TraceTimeline),
  { ssr: false, loading: () => <ListSkeleton count={4} /> },
);

const TraceWaterfallGantt = dynamic(
  () => import('@/components/agent/traces/TraceWaterfallGantt').then((mod) => mod.TraceWaterfallGantt),
  { ssr: false, loading: () => <GraphSkeleton /> },
);

const TokenDecompositionHeatmap = dynamic(
  () => import('@/components/agent/traces/TokenDecompositionHeatmap').then((mod) => mod.TokenDecompositionHeatmap),
  { ssr: false, loading: () => <GraphSkeleton /> },
);

const RawAuditLogViewer = dynamic(
  () => import('@/components/agent/traces/RawAuditLogViewer').then((mod) => mod.RawAuditLogViewer),
  { ssr: false, loading: () => <ListSkeleton count={3} /> },
);
...
export default function DecisionTracePage() {
  ...
  const handleApprove = async (actionId: string, stepId: string) => {
    try {
      ...
      setTrace((prev: AgentTraceResponseData | null) => {
        if (!prev) return prev;
        const updatedSteps = prev.timelineSteps.map((step: TraceStepData) => {
          if (step.metadata?.actionId === actionId) {
            return {
              ...step,
              label: step.label.replace('PENDING', 'APPROVED'),
              metadata: { ...step.metadata, status: 'approved' },
              detail:
                typeof step.detail === 'object' && step.detail !== null
                  ? { ...(step.detail as Record<string, unknown>), status: 'approved' }
                  : step.detail,
            };
          }
          return step;
  ...
  const maxWaterfallDuration = Math.max(
    ...waterfallSpans.map((s: OTelSpanData) => s.startTimeMs + s.durationMs),
    summary.totalLatencyMs || 100,
  );

  return (
    <SectionErrorBoundary sectionName="Decision Trace">
