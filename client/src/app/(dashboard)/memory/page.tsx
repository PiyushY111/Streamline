...
const KnowledgeGraphVisualizer = dynamic(
  () => import('@/components/memory/KnowledgeGraphVisualizer').then((mod) => mod.KnowledgeGraphVisualizer),
  {
    ssr: false,
...
export default function MemoryVaultPage() {
  ...
  const loadData = useCallback(async () => {
    try {
      setLoading(true);
      const [fetchedMemories, fetchedProvider] = await Promise.all([fetchUserMemories(), fetchActiveAiProvider()]);
  ...
  return (
    <SectionErrorBoundary sectionName="Long-Term Memory">
        ...
        <div className="relative overflow-hidden p-8 rounded-3xl clean-card dark:dark-glass dark:dark-glow border border-slate-200 dark:border-slate-800/80 flex flex-col md:flex-row md:items-center justify-between gap-6">
          <div className="space-y-2 max-w-2xl z-10">
            ...
            <p className="text-xs sm:text-sm text-slate-500 dark:text-slate-400 leading-relaxed">
              Persistent episodic memory powering contextual, cross-session agent reasoning. Combines dense HNSW vector
              embeddings with sparse keyword search using Reciprocal Rank Fusion (RRF).
            </p>
          </div>
          ...
          <div className="flex flex-col sm:flex-row items-start sm:items-center gap-3 z-10 shrink-0">
            ...
            <div className="inline-flex items-center space-x-2 px-3.5 py-2 rounded-2xl bg-slate-100 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 text-xs">
              ...
              <div className="flex flex-col text-left">
                <span className="text-[10px] font-bold uppercase tracking-wider text-slate-400">Active AI Engine</span>
                <span className="font-semibold text-slate-800 dark:text-slate-200 flex items-center space-x-1">
                  ...
                  {providerInfo && (
                    <span className="text-[10px] text-purple-500 font-mono">({providerInfo.dimensions}-dim)</span>
                  )}
                </span>
              </div>
            </div>
          ...
          <>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
              ...
              >
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-2xl font-extrabold text-slate-900 dark:text-white">{metrics.total}</span>
                </div>
                <div className="mt-3">
                  <h3 className="text-xs font-bold text-slate-700 dark:text-slate-200">Total Memories</h3>
                  <p className="text-[11px] text-slate-400 dark:text-slate-500 mt-0.5">HNSW vector indexed entries</p>
                </div>
              </div>
              ...
              >
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-2xl font-extrabold text-slate-900 dark:text-white">{metrics.preferences}</span>
                </div>
                <div className="mt-3">
                  <h3 className="text-xs font-bold text-slate-700 dark:text-slate-200">Preferences</h3>
                  <p className="text-[11px] text-slate-400 dark:text-slate-500 mt-0.5">Habits, scheduling, styling</p>
                </div>
              </div>
              ...
              >
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-2xl font-extrabold text-slate-900 dark:text-white">{metrics.decisions}</span>
                </div>
                <div className="mt-3">
              ...
              >
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-2xl font-extrabold text-slate-900 dark:text-white">{metrics.facts}</span>
                </div>
                <div className="mt-3">
                  <h3 className="text-xs font-bold text-slate-700 dark:text-slate-200">Project Facts</h3>
                  <p className="text-[11px] text-slate-400 dark:text-slate-500 mt-0.5">Codebase architecture & rules</p>
                </div>
              </div>
            </div>
              ...
              <div className="p-6 rounded-3xl clean-card dark:dark-glass border border-purple-500/30 shadow-xl space-y-4 animate-in fade-in duration-200">
                <div className="flex items-center justify-between">
                  <div className="flex items-center space-x-2">
                    ...
                    <h2 className="text-sm font-bold text-slate-900 dark:text-white">Record New Long-Term Memory</h2>
                  </div>
                  <button
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                    {MEMORY_TYPES.map((type) => {
                      ...
                      return (
                        ...
                        >
                          <Icon
                            className={`w-4 h-4 mt-0.5 shrink-0 ${isSelected ? 'text-purple-500' : 'text-slate-400'}`}
                          />
                          <div>
              ...
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div className="space-y-1">
                  ...
                  <p className="text-xs text-slate-500 dark:text-slate-400">
                    Inspect real-time dense vector retrieval via Neon pgvector HNSW cosine distance (
                    <code className="font-mono text-indigo-500">&lt;=&gt;</code>) fused with sparse PostgreSQL{' '}
                    <code className="font-mono text-purple-500">tsvector</code> search via Reciprocal Rank Fusion
                    ($k=60$).
                  </p>
                </div>
            ...
            <div className="p-6 rounded-3xl clean-card dark:dark-glass border border-slate-200 dark:border-slate-800/80 space-y-3">
              ...
              <p className="text-xs text-slate-500 dark:text-slate-400 leading-relaxed">
                The Memory Vault is built on Neon PostgreSQL with the{' '}
                <code className="font-mono text-purple-500">pgvector</code> HNSW index and GIN full-text index. When the
                agent uses <code className="font-mono text-purple-500">save_memory</code>, it operates under zero
                real-world blast radius (classified as a read operation in ADR-0010 & ADR-0011) to avoid human
                confirmation friction while maintaining user safety. The memory layer is 100% provider-agnostic,
                supporting Google Gemini, OpenAI-compatible endpoints, and offline mock environments.
              </p>
            </div>
            ...
            {deletingId && (
              <div className="fixed inset-0 bg-slate-950/70 backdrop-blur-xs flex items-center justify-center p-4 z-50 animate-in fade-in duration-150">
                <div className="bg-white dark:bg-[#0D1322] border border-slate-200 dark:border-slate-800 rounded-3xl p-6 w-full max-w-md space-y-4 shadow-2xl">
                  ...
                  <p className="text-xs text-slate-500 dark:text-slate-400 leading-relaxed">
                    Are you sure you want to remove this memory from your long-term vault? The agent will no longer
                    retrieve this information during future planning and synthesis.
                  </p>
