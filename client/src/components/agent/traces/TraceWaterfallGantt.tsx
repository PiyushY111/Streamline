...
export const TraceWaterfallGantt: React.FC<TraceWaterfallGanttProps> = ({ waterfallSpans, maxWaterfallDuration }) => {
  return (
    <div className="p-4 rounded-2xl bg-white/80 dark:bg-[#0B101D]/80 border border-slate-200/80 dark:border-slate-800/80 backdrop-blur-md shadow-xs space-y-4">
      <div className="flex items-center justify-between text-xs text-slate-600 dark:text-slate-300 pb-2 border-b border-slate-200 dark:border-slate-800">
        <span className="font-semibold text-slate-800 dark:text-slate-100">Execution Span Hierarchy</span>
        <span className="font-mono text-slate-600 dark:text-slate-300">
          Time Window: 0ms → {maxWaterfallDuration}ms
        </span>
      </div>
      ...
      <div className="space-y-3">
        {waterfallSpans.map((span) => {
          ...
          let barColor = 'bg-indigo-500 dark:bg-indigo-400';
          if (span.statusCode === 'INTERCEPTED') barColor = 'bg-rose-500 dark:bg-rose-400';
          else if (span.name.includes('memory')) barColor = 'bg-purple-500 dark:bg-purple-400';
          else if (span.name.includes('tool')) barColor = 'bg-amber-500 dark:bg-amber-400';
          else if (span.name.includes('generate')) barColor = 'bg-emerald-500 dark:bg-emerald-400';
