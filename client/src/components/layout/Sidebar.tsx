  ...
  return (
    ...
    >
              ...
              <div className={cn('flex items-center', !isCollapsed && 'space-x-3')}>
                <Icon
                  className={cn(
                    ...
                    isActive
                      ? 'text-white'
                      : 'text-slate-500 dark:text-slate-400 group-hover:text-slate-700 dark:group-hover:text-purple-300',
                  )}
                />
      ...
      <div className="p-3 border-t border-slate-200/80 dark:border-slate-800/60 flex items-center justify-between">
        {!isCollapsed && <span className="text-xs text-slate-500 dark:text-slate-400 font-medium">Theme</span>}
        <ThemeToggle />
      </div>
    </aside>
  );
