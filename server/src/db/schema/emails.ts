...
export const emails = pgTable(
  'emails',
  {
    ...
    attachments:
      jsonb('attachments').$type<
        Array<{ filename: string; mimeType: string; size: number; attachmentId?: string; content?: string }>
      >(),
    createdAt: timestamp('created_at').defaultNow().notNull(),
    updatedAt: timestamp('updated_at').defaultNow().notNull(),
  },
  (table) => ({
    ...
    accountFolderReceivedIdx: index('emails_account_folder_received_idx').on(
      table.accountId,
      table.folder,
      table.receivedAt,
    ),
    ...
    folderCheck: check(
      'emails_folder_valid',
      sql`${table.folder} IN ('inbox', 'sent', 'drafts', 'trash', 'spam', 'archive', 'starred')`,
    ),
    categoryCheck: check(
      'emails_category_valid',
      sql`${table.category} IN ('primary', 'social', 'promotions', 'updates', 'forums')`,
    ),
  }),
);
